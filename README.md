...
```bash
az deployment sub create --name workload-stamp-prereqs --location eastus --template-file ./bicep/workload-stamp-prereqs.bicep
```
...
```bash
az deployment group create -f ./bicep/workload-stamp.bicep -g rg-shipping-dronedelivery -p droneSchedulerPrincipalId=$DRONESCHEDULER_PRINCIPAL_ID \
