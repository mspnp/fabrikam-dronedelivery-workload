...
```bash
DELIVERY_PRINCIPAL_ID=$(az identity show -g rg-shipping-dronedelivery-eastus -n uid-delivery --query principalId -o tsv) && \
DRONESCHEDULER_PRINCIPAL_ID=$(az identity show -g rg-shipping-dronedelivery-eastus -n uid-dronescheduler --query principalId -o tsv) && \
WORKFLOW_PRINCIPAL_ID=$(az identity show -g rg-shipping-dronedelivery-eastus -n uid-workflow --query principalId -o tsv) && \
PACKAGE_ID_PRINCIPAL_ID=$(az identity show -g rg-shipping-dronedelivery-eastus -n uid-package --query principalId -o tsv) && \
INGESTION_ID_PRINCIPAL_ID=$(az identity show -g rg-shipping-dronedelivery-eastus -n uid-ingestion --query principalId -o tsv)
```

### Deploy the workload

```bash
az deployment group create -f ./workload-stamp.bicep -g rg-shipping-dronedelivery-eastus -p droneSchedulerPrincipalId=$DRONESCHEDULER_PRINCIPAL_ID \
...
```bash
ACR_NAME=$(az deployment group show -g rg-shipping-dronedelivery-eastus -n workload-stamp --query properties.outputs.acrName.value -o tsv)
